...
public class UserService 
{
	...
	public void savedetails(Userdto userdto)
	{
		...
		userRegistration.setAccountnumber(userdto.getAccountnumber());
		userRegistration.setAmmount(100.0);
	...
	public String bankingmathod(bankingdto bankingobject,long id)
	{
		double ammount;
		UserRegistration userRegistration=userrepository.findById(id).get();
		if(bankingobject.getDeposit().equalsIgnoreCase("DEPOSIT"))
		{
			ammount= bankingobject.getAmmount()+userRegistration.getAmmount();
		}
		else
		{
			ammount=userRegistration.getAmmount()-bankingobject.getAmmount();
		}
		userRegistration.setAmmount(ammount);
		userrepository.save(userRegistration);
		return "banking is completed";
	}
}
